...
public class UIShopControllerChangePage
{
	...
	static void HandleShop(CustomShop shop, NpcShopTable __result)
	{
	...
	static void HandleAddPage(CustomShopPage page)
	{
		...
		foreach (var item in page.Items)
		{
			...
			ShopTweakPlugin.Log.LogDebug($"NewPage category:{shop.ShopType}, pageName:{catalogPage.name}, itemId:{shopItem.ItemId}, itemLv:{shopItem.itemLv}, prices:{shopItem.prices}");
		}
		...
		if (catalogPage.name.Length > 0 && catalogPage.shopItems.Count > 0)
		{
			...
			ShopTweakPlugin.Log.LogDebug($"NewPage category:{shop.ShopType}, page:{__result.ShopCatalogPages.Count + 1}, pageName:{catalogPage.name}");
		}
		else
		{
			ShopTweakPlugin.Log.LogWarning($"NewPage had no name or items category:{shop.ShopType}");
		}
	}
	...
	static void Postfix(UIShopController __instance)
	{
		...
		var shop = ShopTweakPlugin.Shops.Where(x => x.ShopType == category).FirstOrDefault();
		if (shop is null)
		{

			return;
		}
		HandleShop(shop, __instance.NpcShopTable);
		...
		ShopTweakPlugin.Log.LogDebug($"SetShopTable shopType:{__instance.shopType}, shopPages:{__instance.NpcShopTable.ShopCatalogPages.Count}, pageMax:{__instance.pageMax}, discountRate:{__instance.UIShopControl.discountRate}");

		foreach (ShopCatalogPage page in __instance.NpcShopTable.ShopCatalogPages)
		{
			ShopTweakPlugin.Log.LogDebug($"Page:{page.name}, itemCount:{page.shopItems.Count}");
		}
	}
}
